...
public class OrderServiceImpl implements OrderService {
	...
	public List<DetailOrderItem> compareAmount(PaymentDTO pd, List<DetailOrderItem> itemList, Memberkjy memberInfo) {
		...
		boolean result = false;
		int realAmount = 0; // 검증된 금액
		int totalAmount = 0; // 총 상품 가격
	...
	public Map<String, Object> getPaymentDetail(String orderNo) throws Exception {
		...
		if (completeOrder != null) {
			...
			if (completeOrder.getActualPaymentAmount() != 0) {
				completeOrder
						.setDiscountAmount(completeOrder.getTotalAmount() - completeOrder.getActualPaymentAmount() + completeOrder.getShippingFee());
			} else {
				completeOrder.setDiscountAmount(completeOrder.getTotalAmount() - completeOrder.getAmountToPay() + completeOrder.getShippingFee());
				System.out.println("카드 결제 아닐 때 : " + completeOrder.getAmountToPay());
			}
